...
func (conn *Connection) limbo(sent <-chan PushNotification, responses chan Response, errors chan BadPushNotification, queue chan PushNotification) {
	...
	for {
		...
		case resp, ok := <-responses:
			...
			switch {
			case resp.Status == 0:
				log.Println("Got a 0; flushing old notifications up to:", resp.Identifier)
			...
			FlushLoop:
				for pn := range limbo {
					...
					case pn.Identifier == resp.Identifier:
						log.Println("This notification is the last one we know is OK:", pn.Identifier)
						break FlushLoop
					default:
						log.Println("This notification was sent before a successful one, so it's OK:", pn.Identifier)
						//This one was successful; drop silently.
					}
					if len(limbo) == 0 {
						log.Println("Flushed all notifications.")
						break
					}
				}
			default:
				...
				for pn := range limbo {
					...
					case pn.Identifier != resp.Identifier && !hit:
						log.Println("This notification was sent before a failed one, so it's OK:", pn.Identifier)
						//We haven't seen the identified notification yet
						//so these are all successful (drop silently)
					case pn.Identifier == resp.Identifier:
						log.Println("This notification was a bad one:", pn.Identifier)
						hit = true
						if resp.Status != 10 {
					...
					case pn.Identifier != resp.Identifier && hit:
						log.Println("This notification came after a failed one, so requeue:", pn.Identifier)
